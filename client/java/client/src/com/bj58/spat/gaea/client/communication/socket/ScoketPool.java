...
public class ScoketPool {
	 ...
	 *       3、服务器端接收客户端提供的公钥
	 *       (CPublicKey),并生成新的RSA公(SPublicKey)/私(SPrivateKey)钥,将公钥
	 *       (SPublicKey)传送给客户端 4、客户端用服务器端提供的公钥(SPublicKey)加密授权文件，并传送给服务器端
	 *       5、服务器端通过服务器端私钥
	 *       (SPrivateKey)解密、并校验授权文件是否正确，如果正确则返回通过客户端公钥(CPublicKey)
	 *       加密的RSA密钥，否则返回null/false 6、客户端通过客户端私钥(CPrivateKey)解密服务器端返回数据获得RSA密钥
